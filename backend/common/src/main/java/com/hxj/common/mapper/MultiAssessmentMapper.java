...
public interface MultiAssessmentMapper extends BaseMapper<MultiAssessmentVO> {
     ...
     * @param hasCurb 是否有CURB评估（可选）
     * @param hasPsi 是否有PSI评估（可选）
     * @param hasCpis 是否有CPIS评估（可选）
     * @param hasDiagnosis 是否有重症肺炎诊断（可选）
     * @return 分页查询结果
     */
    @Select("<script>" +
            "SELECT " +
            ...
            "  AND p.age &lt;= #{maxAge} " +
            "</if>" +
            "<if test='hasCurb != null and hasCurb'>" +
            "  AND curb.curb_id IS NOT NULL " +
            "</if>" +
            "<if test='hasPsi != null and hasPsi'>" +
            "  AND psi.psi_id IS NOT NULL " +
            "</if>" +
            "<if test='hasCpis != null and hasCpis'>" +
            "  AND cpis.cpis_id IS NOT NULL " +
            "</if>" +
            "<if test='hasDiagnosis != null and hasDiagnosis'>" +
            "  AND spd.diagnosis_id IS NOT NULL " +
            "</if>" +
            "ORDER BY p.patient_id DESC " +
            "</script>")
    IPage<MultiAssessmentVO> selectMultiAssessmentPage(
            Page<MultiAssessmentVO> page,
            @Param("patientNumber") String patientNumber,
            @Param("patientName") String patientName,
            @Param("gender") String gender,
            @Param("minAge") Integer minAge,
            @Param("maxAge") Integer maxAge,
            @Param("hasCurb") Boolean hasCurb,
            @Param("hasPsi") Boolean hasPsi,
            @Param("hasCpis") Boolean hasCpis,
            @Param("hasDiagnosis") Boolean hasDiagnosis
    );
