...
import com.hxj.service.Covid19AssessmentService;
...
public class Covid19AssessmentController {
    ...
    @Autowired
    private Covid19AssessmentService assessmentService;
    ...
    public Result<Covid19Assessment> assessSinglePatient(@PathVariable Long patientId) {
        try {
            log.info("开始对患者{}进行COVID-19重型诊断", patientId);
            Covid19Assessment assessment = assessmentService.assessSinglePatient(patientId);
            log.info("患者{}COVID-19诊断完成", patientId);
            return Result.success(assessment);
        } catch (Exception e) {
            log.error("患者{}COVID-19诊断失败", patientId, e);
    ...
    public Result<BatchAssessmentResult> assessAllPatients() {
        try {
            ...
            log.info("批量COVID-19诊断完成");
            return Result.success(result);
        } catch (Exception e) {
            log.error("批量COVID-19诊断失败", e);
     ...
     */
    @PostMapping("/page")
    public Result<IPage<Covid19AssessmentPageVO>> queryCovid19Page(@RequestBody Covid19AssessmentQueryDTO queryDTO) {
        
        log.info("分页查询COVID-19重型诊断结果，查询条件: {}", queryDTO);
        ...
        try {
            ...
            if (queryDTO.getSeverityLevel() != null && !queryDTO.getSeverityLevel().isEmpty()) {
                String[] validLevels = {"非重型", "重型", "重型（高危）", "重型（极高危）"};
                if (!Arrays.asList(validLevels).contains(queryDTO.getSeverityLevel())) {
                    return Result.error("严重程度等级必须是：非重型、重型、重型（高危）或重型（极高危）");
                }
            }
            ...
            // 调用Service层执行查询
            IPage<Covid19AssessmentPageVO> result = assessmentService.queryCovid19Page(queryDTO);
            ...
            log.error("分页查询COVID-19重型诊断结果失败", e);
    ...
    public Result<String> health() {
        return Result.success("COVID-19诊断服务运行正常");
    }
}
